...
import { Scissors, LogOut, Users, TrendingUp, Trash2 } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  }, [profile?.id]); // Sadece profile.id değişince çalışsın

  async function loadData() {
    setLoading(true);
    try {
      // 1. Önce employees ve services yükle
      await Promise.all([loadEmployees(), loadServices()]);
      // 2. Sonra transactions yükle
  ...
  async function loadTransactions() {
    ...
    // ✅ Admin tüm transaction'ları çeker
    ...
    if (data && employees.length > 0 && services.length > 0) {
  ...
  // Employees veya services değiştiğinde transactions'ı yeniden hesapla
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <h2 className="text-xl font-bold text-slate-900 mb-4">Çalışan Performansı</h2>

          {employeeStats.length === 0 ? (
            <p className="text-slate-600 text-center py-8">Henüz çalışan kaydı bulunmuyor</p>
          ) : (
